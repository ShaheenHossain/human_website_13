...
from eagle import fields, http, SUPERUSER_ID, tools, _
from eagle.http import request
from eagle.addons.base.models.ir_qweb_fields import nl2br
from eagle.addons.http_routing.models.ir_http import slug
from eagle.addons.payment.controllers.portal import PaymentProcessing
from eagle.addons.website.controllers.main import QueryURL
from eagle.addons.website.models.ir_http import sitemap_qs2dom
from eagle.exceptions import ValidationError
from eagle.addons.website.controllers.main import Website
...
class WebsiteSaleForm(WebsiteForm):
    ...
    @http.route('/website_form/info.sale.order', type='http', auth="public", methods=['POST'], website=True)
...
class Website(Website):
    ...
    def toggle_switchable_view(self, view_key):
        ...
        if view_key in ('website_sale.products_list_view', 'website_sale.add_grid_or_list_option'):
            request.session.pop('website_sale_info_layout_mode', None)
...
class WebsiteSale(http.Controller):
    ...
    def sitemap_info(env, rule, qs):
        if not qs or qs.lower() in '/info':
            yield {'loc': '/info'}

        Category = env['product.public.category']
        dom = sitemap_qs2dom(qs, '/info/category', Category._rec_name)
        dom += env['website'].get_current_website().website_domain()
        for cat in Category.search(dom):
            loc = '/info/category/%s' % slug(cat)
    ...
    @http.route([
        '''/info''',
        '''/info/page/<int:page>''',
        '''/info/category/<model("product.public.category"):category>''',
        '''/info/category/<model("product.public.category"):category>/page/<int:page>'''
    ], type='http', auth="public", website=True, sitemap=sitemap_info)
    def info(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if ppg:
            try:
                ...
                ppg = False
        if not ppg:
            ppg = request.env['website'].get_current_website().info_ppg or 20

        ppr = request.env['website'].get_current_website().info_ppr or 4
        ...
        keep = QueryURL('/info', category=category and int(category), search=search, attrib=attrib_list, order=post.get('order'))
        ...
        if category:
            url = "/info/category/%s" % slug(category)
        ...
        layout_mode = request.session.get('website_sale_info_layout_mode')
        if not layout_mode:
    ...
    @http.route(['/info/details/<model("product.template"):product>'], type='http', auth="public", website=True)
    ...
    def _prepare_product_values(self, product, category, search, **kwargs):
        ...
        keep = QueryURL('/info', category=category and category.id, search=search, attrib=attrib_list)
    ...
    @http.route(['/info/change_pricelist/<model("product.pricelist"):pl_id>'], type='http', auth="public", website=True, sitemap=False)
    def pricelist_change(self, pl_id, **post):
        ...
        return request.redirect(request.httprequest.referrer or '/info')

    @http.route(['/info/pricelist'], type='http', auth="public", website=True, sitemap=False)
    def pricelist(self, promo, **post):
        redirect = post.get('r', '/info/cart')
    ...
    @http.route(['/info/cart'], type='http', auth="public", website=True, sitemap=False)
    def cart(self, access_token=None, revive='', **post):
        ...
        if access_token:
            ...
            if abandoned_order.state != 'draft':  # abandoned cart already finished
                ...
                return request.redirect('/info/cart')
            elif revive == 'merge':
    ...
    @http.route(['/info/cart/update'], type='http', auth="public", methods=['GET', 'POST'], website=True, csrf=False)
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if kw.get('express'):
            return request.redirect("/info/checkout?express=1")

        return request.redirect("/info/cart")

    @http.route(['/info/cart/update_json'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    ...
    @http.route('/info/save_info_layout_mode', type='json', auth='public', website=True)
    def save_info_layout_mode(self, layout_mode):
        assert layout_mode in ('grid', 'list'), "Invalid info layout mode"
        request.session['website_sale_info_layout_mode'] = layout_mode
    ...
    def checkout_redirection(self, order):
        ...
        if tx and tx.state != 'draft':
            return request.redirect('/info/payment/confirmation/%s' % order.id)
    ...
    @http.route(['/info/address'], type='http', methods=['GET', 'POST'], auth="public", website=True, sitemap=False)
    def address(self, **kw):
        ...
        # IF ORDER LINKED TO A PARTNER
            ...
            if partner_id > 0:
                ...
                return request.redirect('/info/checkout')
                ...
                if mode[1] == 'billing':
                    ...
                    if not kw.get('use_same'):
                        kw['callback'] = kw.get('callback') or \
                            (not order.only_services and (mode[0] == 'edit' and '/info/checkout' or '/info/address'))
                elif mode[1] == 'shipping':
                    order.partner_shipping_id = partner_id

                order.message_partner_ids = [(4, partner_id), (3, request.website.partner_id.id)]
                if not errors:
                    return request.redirect(kw.get('callback') or '/info/confirm_order')
    ...
    @http.route(['/info/checkout'], type='http', auth="public", website=True, sitemap=False)
    def checkout(self, **post):
        ...
        if order.partner_id.id == request.website.user_id.sudo().partner_id.id:
            return request.redirect('/info/address')

        for f in self._get_mandatory_billing_fields():
            if not order.partner_id[f]:
                return request.redirect('/info/address?partner_id=%d' % order.partner_id.id)

        values = self.checkout_values(**post)

        if post.get('express'):
            return request.redirect('/info/confirm_order')
    ...
    @http.route(['/info/confirm_order'], type='http', auth="public", website=True, sitemap=False)
    def confirm_order(self, **post):
        ...
        if extra_step.active:
            return request.redirect("/info/extra_info")

        return request.redirect("/info/payment")

    # ------------------------------------------------------
    # Extra step
    # ------------------------------------------------------
    @http.route(['/info/extra_info'], type='http', auth="public", website=True, sitemap=False)
    ...
    def _get_info_payment_values(self, order, **kwargs):
        values = dict(
            website_sale_order=order,
            errors=[],
            partner=order.partner_id.id,
            ...
            return_url= '/info/payment/validate',
    ...
    @http.route(['/info/payment'], type='http', auth="public", website=True, sitemap=False)
    def payment(self, **post):
        ...
        render_values = self._get_info_payment_values(order, **post)
    ...
    @http.route(['/info/payment/transaction/',
        '/info/payment/transaction/<int:so_id>',
        '/info/payment/transaction/<int:so_id>/<string:access_token>'], type='json', auth="public", website=True)
    ...
    @http.route('/info/payment/token', type='http', auth='public', website=True, sitemap=False)
    def payment_token(self, pm_id=None, **kwargs):
        ...
        if not order:
            return request.redirect('/info/?error=no_order')
        ...
        try:
            ...
            return request.redirect('/info/?error=invalid_token_id')

        # We retrieve the token the user want to use to pay
        if not request.env['payment.token'].sudo().search_count([('id', '=', pm_id)]):
            return request.redirect('/info/?error=token_not_found')

        # Create transaction
        vals = {'payment_token_id': pm_id, 'return_url': '/info/payment/validate'}

        tx = order._create_payment_transaction(vals)
        PaymentProcessing.add_payment_transaction(tx)
        return request.redirect('/payment/process')

    @http.route('/info/payment/get_status/<int:sale_order_id>', type='json', auth="public", website=True)
    ...
    @http.route('/info/payment/validate', type='http', auth="public", website=True, sitemap=False)
    def payment_validate(self, transaction_id=None, sale_order_id=None, **post):
        ...
        return request.redirect('/info/confirmation')


    @http.route(['/info/terms'], type='http', auth="public", website=True)
    def terms(self, **kw):
        return request.render("website_sale.terms")

    @http.route(['/info/confirmation'], type='http', auth="public", website=True, sitemap=False)
    ...
    @http.route(['/info/print'], type='http', auth="public", website=True, sitemap=False)
    ...
    @http.route(['/info/tracking_last_order'], type='json', auth="public")
    ...
    @http.route(['/info/add_product'], type='json', auth="user", methods=['POST'], website=True)
    ...
    @http.route(['/info/change_styles'], type='json', auth='user')
    ...
    @http.route(['/info/change_sequence'], type='json', auth='user')
    ...
    @http.route(['/info/change_size'], type='json', auth='user')
    def change_size(self, id, x, y):
        product = request.env['product.template'].browse(id)
        return product.write({'website_size_x': x, 'website_size_y': y})

    @http.route(['/info/change_ppg'], type='json', auth='user')
    def change_ppg(self, ppg):
        request.env['website'].get_current_website().info_ppg = ppg

    @http.route(['/info/change_ppr'], type='json', auth='user')
    def change_ppr(self, ppr):
        request.env['website'].get_current_website().info_ppr = ppr
    ...
    @http.route(['/info/country_infos/<model("res.country"):country>'], type='json', auth="public", methods=['POST'], website=True)
    ...
    @http.route('/info/products/autocomplete', type='json', auth='public', website=True)
    ...
    # --------------------------------------------------------------------------
    @http.route('/info/products/recently_viewed', type='json', auth='public', website=True)
    ...
    @http.route('/info/products/recently_viewed_update', type='json', auth='public', website=True)
    ...
    @http.route('/info/products/recently_viewed_delete', type='json', auth='public', website=True)
