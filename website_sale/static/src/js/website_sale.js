eagle.define('website_sale.cart', function (require) {
'use strict';
...
publicWidget.registry.websiteSaleCartLink = publicWidget.Widget.extend({
    selector: '#top_menu a[href$="/info/cart"]',
    events: {
    ...
    _onMouseEnter: function (ev) {
        ...
        timeout = setTimeout(function () {
            ...
            self._popoverRPC = $.get("/info/cart", {
                type: 'popover',
            }).then(function (data) {
...
eagle.define('website_sale.website_sale_category', function (require) {
'use strict';

var publicWidget = require('web.public.widget');

publicWidget.registry.websiteSaleCategory = publicWidget.Widget.extend({
    selector: '#o_info_collapse_category',
    events: {
...
eagle.define('website_sale.website_sale', function (require) {
'use strict';
...
publicWidget.registry.WebsiteSale = publicWidget.Widget.extend(VariantMixin, {
    ...
    _changeCartQuantity: function ($input, value, $dom_optional, line_id, productIDs) {
        ...
        this._rpc({
            ...
            if (!data.cart_quantity) {
                return window.location = '/info/cart';
            }
    ...
    _changeCountry: function () {
        ...
        this._rpc({
            route: "/info/country_infos/" + $("#country_id").val(),
            params: {
                mode: 'shipping',
            },
        }).then(function (data) {
    ...
    /**
     * This is overridden to handle the "List View of Variants" of the web info.
        ...
        if (!config.device.isMobile) {
            ...
            _.each($('.ecom-zoomable img[data-zoom]'), function (el) {
                onImageLoaded(el, function () {
                    var $img = $(el);
                    $img.zoomEagle({event: autoZoom ? 'mouseenter' : 'click', attach: attach});
     ...
     *
     * @private
     * @param {$.Element} $form the related webinfo form
     */
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (!this.editableMode) {
            this._rpc({
                route: '/info/save_info_layout_mode',
                params: {
...
publicWidget.registry.websiteSaleCart = publicWidget.Widget.extend({
    ...
    _onClickEditAddress: function (ev) {
        ev.preventDefault();
        $(ev.currentTarget).closest('div.one_kanban').find('form.d-none').attr('action', '/info/address').submit();
    },
    /**
     * @private
     * @param {Event} ev
...
publicWidget.registry.productsSearchBar = publicWidget.Widget.extend({
    ...
    _fetch: function () {
        return this._rpc({
            route: '/info/products/autocomplete',
            params: {
                'term': this.$input.val(),
                'options': {
                    'order': this.order,
